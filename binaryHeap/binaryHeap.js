...
var minCompare = function(i, j) {
  return i < j;
};

var maxCompare = function(i, j) {
  return i > j;
}
...
BinaryHeap.prototype.insert = function (value) {
  ...
  // if it's smaller than parent, then swap with parent
  ...
  while(parentIndex > -1) {
    ...
    if (this._compare(this._heap[index], this._heap[parentIndex])) {
      // swap
      console.log('swap', this._heap[index], this._heap[parentIndex]);
...
BinaryHeap.prototype.removeRoot = function () {
  console.log('REMOVE ROOT', this._heap);
  // move last element to root and delete last one
  var removedRoot = this._heap[0];
  this._heap[0] = this._heap[this._heap.length-1];
  this._heap.splice(this._heap.length-1, 1);


  var childrenIndices = [1, 2];
  var parentIndex = 0;

  while(childrenIndices && childrenIndices.length > 0) {
    if(childrenIndices.length === 2) {
      var minChildIndex = this._compare(this._heap[childrenIndices[0]], this._heap[childrenIndices[1]]) ? childrenIndices[0] : childrenIndices[1];
    } else {
      minChildIndex = childrenIndices[0];
    }

    if(this._compare(this._heap[minChildIndex], this._heap[parentIndex])) {
      console.log('swap', this._heap[minChildIndex], this._heap[parentIndex]);
      var temp = this._heap[minChildIndex];
      this._heap[minChildIndex] = this._heap[parentIndex];
      this._heap[parentIndex] = temp;
    }
    parentIndex = minChildIndex;
    childrenIndices = [];
    // console.log(this._heap.length);
    if(this._heap.length - 1 >= minChildIndex * 2 + 1) {
      childrenIndices.push(minChildIndex * 2 + 1);
      if(this._heap.length - 1 >= minChildIndex * 2 + 2) {
        childrenIndices.push(minChildIndex * 2 + 2);
      }
    }

    return removedRoot;
    // console.log('parentIndex', parentIndex);
    // console.log('childrenIndices', childrenIndices);
  }
...
BinaryHeap.prototype.heapSort = function(array) {
  var heap = new BinaryHeap(minCompare);

  heap.forEach(function(value) {
    heap.insert(value);
  });

  var sorted = [];
  while(heap.length > 0) {
    sorted.push(heap.removeRoot());
  }

  return sorted;
}

// var bh = new BinaryHeap(minCompare);
// var bh = new BinaryHeap(maxCompare);
// bh.insert(10);
// bh.insert(9);
// bh.insert(8);
// bh.insert(7);
